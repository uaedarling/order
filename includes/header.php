...
<div class="flex h-full min-h-screen">
  ...
  <aside id="sidebar"
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
      <?= navLink('/pages/dashboard.php', 'layout-dashboard', 'Dashboard',
            $currentPage === 'dashboard.php') ?>
      <?= navLink('/pages/new_order.php', 'plus-circle', 'New Order',
            $currentPage === 'new_order.php') ?>
      <?= navLink('/pages/estimator.php', 'calculator', 'Estimator',
      ...
      <?= navLink('/pages/brands.php', 'building-2', 'Brands',
            $currentPage === 'brands.php') ?>
      <?= navLink('/pages/settings.php', 'settings', 'Settings',
  ...
  <div class="flex-1 flex flex-col min-w-0">
    ...
    <header class="h-16 bg-white border-b border-gray-200 flex items-center gap-4 px-4 lg:px-6 flex-shrink-0">
      ...
      <?php
        ...
        if ($n > 0):
      ?>
      <a href="/pages/dashboard.php?filter=ship_out" class="flex items-center gap-1 px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold">
